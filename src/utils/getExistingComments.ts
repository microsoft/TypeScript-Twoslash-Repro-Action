...
export const getResultCommentInfoForRequest = (comments: Issue['comments']['nodes'], request: TwoslashRequest): InfoComment<EmbeddedTwoslashResult> | undefined => {
  return findTypeScriptBotComment(comments, (info): info is EmbeddedTwoslashResult => info.kind === 'twoslash-result' && info.requestCommentId === request.commentID)
}
...
export function getBisectCommentInfoForRequest(comments: Issue['comments']['nodes'], request: TwoslashRequest): InfoComment<EmbeddedBisectResult> | undefined {
